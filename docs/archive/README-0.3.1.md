...
| a needle |  needle  |  0.221 uSv/h                 |  well it is known
| per tube |    3s    |    3s  70.0  0.455  37.8 pc  |
|          | pointer  |   30s  34.0  0.221  17.1 pc  |  per window, with the
|          |   30s    |  300s  23.0  0.150   6.6 pc  |  precision of each
+----------+----------+------------------------------+
    A B     1s 40 30s 34                               the plates, under
  raw . 30s 3s held . 20-70                              each bezel
   A 60  B 8                                           every tube on its own
   now 0   run 2468 in 2577s   2 tubes, interleave 0.54s
 8s/bar    4s/bar    2s/bar    1s/bar             1/2s  the cascade's tiers,
                                                          the last one four
                                                          seconds of arrival
...
| **COLLECTED** | One needle, and it carries three time constants at once. This is *what is the room doing?* |

They share a scale, because two dials that don't cannot be compared — which is
the only reason to draw them side by side.

### Three time constants on one face

**A dial can show more than one answer at a time and a number cannot** — which
is most of the argument for drawing a dial at all. The collected face carries:

| | |
|---|---|
| **the needle** | **three seconds**, heavy and tapered, and it *moves* rather than jumping |
| **the chrome pointer** | **half a minute** — the same window as the big number, so the dial and the headline agree by construction |
| **the arcs outside the bands** | where the needle has been over the last half minute and minute |

The gap between the needle and the pointer **is the trend**: needle above
pointer is a rising room, and the two together are the reading and its context
in one glance. The one-second figure has not gone anywhere — it is printed
under the face, which is where a number that jumps several times a second
belongs.

**Three seconds and not one.** One second of one tube is a handful of arrivals
— at background, two or three — so a one-second needle draws the counting
statistics rather than the room. Three seconds is the shortest window the rest
of the program already keeps, it is what the top line of figures reports, and
at three times the counts it is √3 steadier: a needle you can read a value
off, still quick enough to show a source passing under the tube.

**And it no longer waits for the second after it.** The reading used to close a
second only when the *first sample of the next one* arrived — so the needle
always showed a second that had already finished, and with several tubes it
was whichever tube ticked over first that ended the wait. The meters now roll
over the arrivals themselves and have the second the moment it has gone by,
whatever the tube count.
...
**The needle is the other half of the same idea.** A reading is not a number,
it is a number with a history, and an instrument that teleports between them
throws the history away. So the needle has mass: it leans out fast — a quarter
of a second, quick enough that a source passing under the tube is not smoothed
away — and settles back slower, about nine tenths, which is what stops one
Poisson lump reading as a spike. It is what every moving-coil meter does
mechanically and what every VU meter specifies deliberately, and without it
the eye cannot follow which way the needle went.

It moves **twelve times a second, and only when it has moved**: a settled
reading sends nothing between the once-a-second snapshots, because this
program's usual home is a VM with no GPU where every update is a full software
